...
proj_label <- "Combined_LeukoComp_450K_Tab1"
...
datatab1sel <- CombPheno2wcue2 %>% filter(!is.na(PTSDlife)) %>% dplyr::select(one_of("Study", "Sex", "Race", "Age", "Smoking_pm","PTSDlife")) %>% dplyr::rename(PTSD = PTSDlife, Smoking = Smoking_pm)
...
table_data[["Lifetime PTSD"]] <- getT1Stat("PTSD")
